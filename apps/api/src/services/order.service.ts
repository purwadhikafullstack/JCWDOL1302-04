...
export class OrderService {
  static async getOrdersByClientOrderId(orderId: string) {
    const orders =
      (await prisma.$queryRaw`SELECT o.id as orderId, o.status, o.totalPayment,
    ...
    const countDay = countDaysInRange(
      new Date(orders[0].checkIn),
      new Date(orders[0].checkOut),
    );
    orders[0].totalDays = countDay;

    const rooms =
      (await prisma.$queryRaw`SELECT ors.quantity, ors.price as totalPrice, rp.price, r.type, r.image, sp.price as specialPrice
      FROM orderRooms ors
      INNER JOIN rooms r ON ors.room_id=r.id
      LEFT JOIN (
  ...
  static async getOrdersByUserId(userId: string) {
    const orders =
      ...
      o.status, o.totalPayment, o.checkIn, o.checkOut, o.invoiceId, o.isReminded, o.paymentProofImage, u.name as customerName,
  ...
  static async reminderOrder(req: CancelOrderReq) {
    const { userId, tenantId, invoiceId } = req;
